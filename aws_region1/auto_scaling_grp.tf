data "aws_ami" "amazon_linux_2" {
  ...
  filter {
    name   = "image-id"
    values = ["ami-037d218ef5bf9babc"]  # The exact AMI ID
  }
...
resource "aws_launch_template" "ec2_asg" {
  name                  = "my-launch-template_region1"
  image_id              = data.aws_ami.amazon_linux_2.id
  instance_type         = "t2.large"
  iam_instance_profile {
    name = "ag_iam_region1"
  }
  user_data = base64encode(templatefile("userdata.sh", { postgress_url = aws_rds_cluster.aurora_cluster.endpoint }))
...
resource "aws_autoscaling_group" "asg-tf" {
  name                 = "autoscaling_in_private_region1"
  desired_capacity     = 1
  max_size             = 1
  min_size             = 1
  force_delete         = true
  ...
  tag {
    key                 = "ag_key"
    value               = "autoscaling_in_private_region1"
